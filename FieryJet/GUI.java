...
private boolean fixed;
  ...
  public GUI(Sprite backgroundSprite, GUIButton[] buttons, int x, int y,
  boolean fixed){
  ...
  public void render(RenderHandler renderer, int xZoom, int yZoom){
    if (backgroundSprite != null){
      renderer.renderSprite(backgroundSprite, rect.x, rect.y, xZoom, yZoom,
      fixed);
    }
  ...
  int xZoom, int yZoom){
    ...
    if (!fixed){
      mouseRectangle = new Rectangle(mouseRectangle.x + camera.x,
      mouseRectangle.y + camera.y, 1, 1);
    } else{
    ...
    if (rect.w == 0 || rect.h == 0 || mouseRectangle.intersects(rect)){
      ...
      for (int i = 0; i < buttons.length; i++){
        boolean result = buttons[i].handleMouseClick(mouseRectangle, camera,
        xZoom, yZoom);
