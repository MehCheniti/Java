...
private boolean isGreen = false;
  ...
  public SDKButton(Game game, int tileID, Sprite tileSprite, Rectangle rect){
    ...
    this.game = game;
    this.tileID = tileID;
    rect.generateGraphics(0xFFFF00);
  }
  ...
  Rectangle interfaceRect){
    ...
    renderer.renderSprite(sprite, rect.x + interfaceRect.x + (xZoom - (xZoom -
    1)) * rect.w / 2 / xZoom, rect.y + interfaceRect.y + (yZoom - (yZoom - 1))
    * rect.h / 2 / yZoom, xZoom - 1, yZoom - 1, fixed);
  }
  ...
  public void update(Game game){
    if (tileID == game.getSelectedTile()){
      if (!isGreen){
        rect.generateGraphics(0x008000);
        isGreen = true;
      }
    } else{
      if (isGreen){
